...
static void outs(FILE *outp, char *str);
...
int main(int argc, void *argv[])
{
	...
	char c_var=0, c_val=0, c_str=0;
	char s_var[SYMCHARS], s_val[SYMCHARS], s_str[SYMCHARS];
	...
	while (c = fgetc(filein)) {
		...
		DP("aline.wp = %d\n", aline.wp);
		DP("Raw    : "); outs(outp, linebuf);

		/*
		 * Make raw tokens from captured characters
		 */
		...
		/*
		 * Make cooked tokens from raw tokens with lexical analysis
		 */
		lex_init();
		for (k=0; k<arg.c; k++) {
			ret = lex_analyze_token(arg.v[k], &token[tokens]);
			DP("%d (%s) = lex_anlyze_token(%s, &token)\n",
			   ret, lex_status_str[ret], arg.v[k]);
...
parse:
	...
	while (k < tokens || token[k].t_type != T_UNDEF) {
		...
		if (ret == PARINVLD) {
			EP("Invalid grammer");
			EP("Debug par.c with setting DEBUG_PAR=1 of include/esh_debug.h");
			exit(EXIT_FAILURE);
		}
		k++;
	}
...
execute:
	...
	// Check end of code
	if (code.c_code[exe.e_pc] == OP_NOP) {
		DP("All codes were successfully executed.\n");
	} else {
		DP("Code execution may have some of problems.\n");
	}
