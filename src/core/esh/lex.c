...
struct lex {
	...
	enum lex_state l_sprev; /* Previous state */
	enum token_type l_tok;  /* Token type */
	char l_bquote;		/* Back QUOTE */
	char l_brack;		/* BRACKet */
	char l_while;		/* While statement */
	char l_scats;		/* The number of concatinated strings */
} lex;
	...
	case LEX_IDLE:
		...
		if (c == '`') {
			...
			*type = T_LBRACKET;
			...
			DP("Single quote: %s",str);
			lex.l_scats = 0;
			*type = T_STRING;
			ret = LEXQUOTE;
			state = LEX_SQUOTE;
			lex.l_sprev = LEX_IDLE;
			lex.l_tok = T_STRING;
		} else if (isdigit(c)) {
			DP("Value: %s",str);
	...
	case LEX_CMD_ARG:
		...
		if (c == ' ' || c == '\0') {
			...
			*type = T_SEMICOLON;
			...
			DP("Single quote in argument vector: %s",str);
			lex.l_scats = 0;
			*type = T_CMD_ARG;
			ret = LEXQUOTE;
			state = LEX_SQUOTE;
			lex.l_sprev = LEX_CMD_ARG;
			lex.l_tok = T_CMD_ARG;
		}
		break;
	case LEX_SQUOTE:
		...
		if (c == '\'') {
			...
			DP("concatinate strings within single quote\n");
			if (lex.l_scats == 0) {
				strncpy(sym, str, SYMCHARS);
				DP("sym=%s",sym);
			} else {
				memset(scat, 0, sizeof(scat));
				strncpy(scat, sym, SYMCHARS);
				snprintf(sym, SYMCHARS, "%s %s", scat, str);
				DP("sym=%s",sym);
			}
			lex.l_scats++;
			*type = lex.l_tok;
		...
		break;
	default:
		EP("%s","lex is now in unknown state", lex_state_str[state]);
		goto error;
	}
